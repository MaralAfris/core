...
6. The container is set to respawn when it stops. This overcomes some issues when starting.
   If it is wanted that the container should only run once the the script will have to be
   manually adjusted.
7. Docker uses inotify and if you are launching many containers it may fail to start on some
   due to a limit.
